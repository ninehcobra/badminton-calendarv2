...
export const UserProfile = ({ userId, isOwnProfile = false }: UserProfileProps) => {
    ...
    const { data: profile, isLoading } = useGetProfileQuery(targetUserId || '', {
        skip: !targetUserId
    });
    const { data: matches, isLoading: isLoadingMatches } = useGetMatchHistoryQuery(targetUserId || '', {
        skip: !targetUserId
    });
    ...
    const wins = matches?.filter((m: MatchResult) => {
        const isTeamA = m.team_a_ids.includes(targetUserId || '');
                ...
                {isLoadingMatches ? (
                    ...
                    <div className="space-y-3">
                        {matches.map((match: MatchResult) => {
                            const isTeamA = match.team_a_ids.includes(targetUserId || '');
